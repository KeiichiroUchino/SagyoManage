...
{
    /// <summary>
    /// 作業小分類テーブルのデータアクセスレイヤです。
    /// </summary>
    public class SagyoShoBunrui
    {
        /// <summary>
        /// 一括処理件数
        /// </summary>
        ...
        private AppAuthInfo _authInfo =
         ...
         /// <summary>
        /// 作業小分類クラスのデフォルトコンストラクタです。
        /// </summary>
        public SagyoShoBunrui()
        {
        }

        /// <summary>
        /// アプリケーション認証情報を指定して、作業小分類テーブルの
        /// データアクセスレイヤのインスタンスを初期化します。
        /// </summary>
        /// <param name="authInfo">アプリケーション認証情報</param>
        public SagyoShoBunrui(AppAuthInfo authInfo)
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public SagyoShoBunruiInfo GetInfo(int code, SqlTransaction transaction = null)
        {
            return GetListInternal(transaction, new SagyoShoBunruiSearchParameter()
            {
                SagyoShoBunruiCode = code,
            }).FirstOrDefault();
        }
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public SagyoShoBunruiInfo GetInfoById(decimal id, SqlTransaction transaction = null)
        {
            return GetListInternal(transaction, new SagyoShoBunruiSearchParameter()
            {
                SagyoShoBunruiId = id,
        ...
        /// <summary>
        /// 検索条件情報を指定して、作業小分類情報のリストを取得します。
        /// </summary>
        /// <param name="para">検索条件情報</param>
        /// <returns>作業小分類情報のリスト</returns>
        public IList<SagyoShoBunruiInfo> GetList(SagyoShoBunruiSearchParameter para = null)
        {
        ...
        public List<SagyoShoBunruiInfo> BulkGetListInternal(List<string> list,
            SqlTransaction transaction = null, bool exclusiveflg = false)
        {
            String mySql = string.Empty;

            List<SagyoShoBunruiInfo> resultList = new List<SagyoShoBunruiInfo>();

            // 該当データがない場合は終了
            if (list.Count() == 0) return new List<SagyoShoBunruiInfo>();
            ...
            foreach (string id in list)
            {
                ...
                sb.AppendLine("	* ");
                sb.AppendLine("FROM ");
                sb.AppendLine(" SagyoShoBunrui ");

                if (exclusiveflg)
                {
                    sb.AppendLine(" SagyoShoBunrui WITH (UPDLOCK,ROWLOCK) ");
                    sb.AppendLine("WHERE ");
                ...
                else
                {
                    ...
                    sb.AppendLine("WHERE ");
                    sb.AppendLine("	DelFlag = " + NSKUtil.BoolToInt(false) + " ");
                    sb.AppendLine("AND SagyoShoBunruiCode = N'" + SQLHelper.GetSanitaizingSqlString(id.Trim()) + "' ");
                }
                ...
                if (i == BLUK_COUNT)
                {
                    ...
                    {
                        //返却用の値
                        SagyoShoBunruiInfo rt_info = new SagyoShoBunruiInfo
                        {
                            ...
                            SagyoShoBunruiCode = SQLServerUtil.dbInt(rdr["SagyoShoBunruiCode"]),
        ...
        public void BulkSave(SqlTransaction transaction, List<SagyoShoBunruiInfo> list)
        {
            ...
            IEnumerator<decimal> meisaiId_iterator =
                SQLHelper.GetSequenceIds(
                    SQLHelper.SequenceIdKind.SagyoShoBunrui, list.Count(x => !x.DelFlag && x.VersionColumn == null)).GetEnumerator();
            ...
            foreach (var info in list)
            {
                ...
                else
                {
                    ...
                    info.SagyoShoBunruiId = meisaiId_iterator.Current;

                    //INSERT
                    commands.Add(this.CreateInCommands(info));
                }
                codeList.Add(info.SagyoShoBunruiCode.ToString());
        ...
        /// <param name="para">検索条件情報</param>
        /// <returns>情報リスト</returns>
        public IList<SagyoShoBunruiInfo> GetListInternal(SqlTransaction transaction, SagyoShoBunruiSearchParameter para)
        {
            //SQL文を作成
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("SELECT ");
            ...
            sb.AppendLine(" SagyoShoBunrui ");
            sb.AppendLine("WHERE ");
            sb.AppendLine("	SagyoShoBunrui.DelFlag = " + NSKUtil.BoolToInt(false) + " ");

            if (para != null)
            {
                if (para.SagyoShoBunruiId.HasValue && para.SagyoShoBunruiId > 0)
                {
                    sb.AppendLine("AND SagyoShoBunrui.SagyoShoBunruiId = " + para.SagyoShoBunruiId.ToString() + " ");
                }
                if (para.SagyoDaiBunruiId.HasValue)
                {
                    sb.AppendLine("AND SagyoShoBunrui.SagyoDaiBunruiId = " + para.SagyoDaiBunruiId.ToString() + " ");
                }
                if (para.SagyoChuBunruiId.HasValue)
                {
                    sb.AppendLine("AND SagyoShoBunrui.SagyoChuBunruiId = " + para.SagyoChuBunruiId.ToString() + " ");
                }
                if (para.SagyoShoBunruiCode.HasValue && para.SagyoShoBunruiCode > 0)
                {
                    sb.AppendLine("AND SagyoShoBunrui.SagyoShoBunruiCode = " + para.SagyoShoBunruiCode.ToString() + " ");
                }
                if (para.DisableFlag.HasValue)
                {
                    sb.AppendLine("AND SagyoShoBunrui.DisableFlag = " + NSKUtil.BoolToInt(para.DisableFlag.Value).ToString() + " ");
                }
            }

            sb.AppendLine("ORDER BY ");
            sb.AppendLine(" SagyoShoBunrui.SagyoShoBunruiCode ");
        ...
        /// <param name="info">得意先</param>
        /// <returns></returns>
        private SqlCommand CreateUpCommands(SagyoShoBunruiInfo info, bool exclusiveflg = true)
        {
            //常設列の取得オプションを作る
            //--更新は入力と更新
            SQLHelper.PopulateColumnOptions popOption =
            ...
            sb.AppendLine(" ,SagyoShoBunruiCode =  " + info.SagyoShoBunruiCode + " ");
            sb.AppendLine(" ,SagyoShoBunruiName = N'" + SQLHelper.GetSanitaizingSqlString(info.SagyoShoBunruiName.Trim()) + "' ");
        ...
        /// <param name="info">得意先</param>
        /// <returns></returns>
        private SqlCommand CreateInCommands(SagyoShoBunruiInfo info)
        {
            //常設列の取得オプションを作る
            //--更新は入力と更新
            SQLHelper.PopulateColumnOptions popOption =
            ...
            sb.AppendLine(" ,SagyoShoBunruiCode  ");
            sb.AppendLine(" ,SagyoShoBunruiName  ");
            ...
            sb.AppendLine(" , " + info.SagyoShoBunruiCode.ToString() + " ");
            sb.AppendLine(" , N'" + SQLHelper.GetSanitaizingSqlString(info.SagyoShoBunruiName.Trim()) + "' ");
