...
namespace Jpsys.HaishaManageV10.ReportDAL
{
    ...
    public class HaisoShijiShoRpt
    {
        ...
        public HaisoShijiShoRpt(AppAuthInfo authInfo)
        {
        ...
        public List<HaisoShijiShoRptInfo> GetHaisha(HaisoShijiShoRptSearchParameter para = null)
        {
            // クエリ
            string query = this.GetQueryHaishaSelect(para);
            List<HaisoShijiShoRptInfo> list = SQLHelper.SimpleRead(query, rdr =>
            {
                //返却用の値
                HaisoShijiShoRptInfo ret = new HaisoShijiShoRptInfo();

                //配車情報
                HaisoShijiShoRptInfo retHaisha = new HaisoShijiShoRptInfo();
                ...
                retHaisha.BranchOfficeName = rdr["BranchOfficeSNM"].ToString();
        ...
        /// <returns></returns>
        private string GetQueryHaishaSelect(HaisoShijiShoRptSearchParameter para)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine(" SELECT ");
            ...
            if (para != null)
            {
                ...
                {
                    sb.AppendLine("   AND Staff.StaffCd >= " + para.StaffCodeFrom.ToString() + " ");
                    sb.AppendLine("   AND Staff.StaffCd <= " + para.StaffCodeTo.ToString() + " ");
                }

                // 乗務員（個別指定）
                if (para.Staff_KobetsuShiteiChecked && para.StaffCheckList.Length != 0)
                {
                    sb.AppendLine("   AND Staff.StaffId IN ( " + para.StaffCheckList + ") ");
                }

                // 車種（範囲指定）
                if (para.CarKind_HaniShiteiChecked)
                {
                    sb.AppendLine("   AND CarKind.CarKindCd >= " + para.CarKindCodeFrom.ToString() + " ");
                    sb.AppendLine("   AND CarKind.CarKindCd <= " + para.CarKindCodeTo.ToString() + " ");
                }

                // 車種（個別指定）
                if (para.CarKind_KobetsuShiteiChecked && para.CarKindCheckList.Length != 0)
                {
                    sb.AppendLine("   AND CarKind.CarKindId IN ( " + para.CarKindCheckList + ") ");
                }
