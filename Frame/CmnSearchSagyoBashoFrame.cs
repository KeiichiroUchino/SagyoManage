...
{
    public partial class CmnSearchSagyoBashoFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "作業場所検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Jpsys.SagyoManage.SQLServerDAL.SagyoBasho searchTblClass;

        /// <summary>
        /// 作業場所情報のリストを保持する領域
        /// </summary>
        private IList<SagyoBashoInfo> SagyoBashoList = null;
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public CmnSearchSagyoBashoFrame()
        {
            InitializeComponent();
        }
        ...
        private void InitCmnSearchSagyoBashoFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new Jpsys.SagyoManage.SQLServerDAL.SagyoBasho();
        ...
        /// メニュー関連を初期化します。
        /// </summary>
        private void InitMenuSagyoBasho()
        {
            //操作メニュー
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtSagyoBashoMei, ctl => ctl.Text, this.edtSagyoBashoMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //作業場所一覧を取得
            this.SagyoBashoList =
                this.searchTblClass.GetList(new SagyoBashoSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.SagyoBashoList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<SagyoBashoInfo> wk_list =
                    this.GetMatchedList(this.SagyoBashoList).OrderBy(x => x.SagyoBashoCode).ToList();
                ...
                {
                    SagyoBashoInfo wk_info = wk_list[j];
                    ...
                    datamodel.SetValue(j, (int)SpreadColKeys.SagyoBashoAddress1, wk_info.SagyoBashoAddress1);
                    datamodel.SetValue(j, (int)SpreadColKeys.SagyoBashoAddress2, wk_info.SagyoBashoAddress2);
                    datamodel.SetValue(j, (int)SpreadColKeys.ShiyoTeishi, wk_info.ShiyoTeishi);

                    // Tagへ1件分のデータを退避（後で使用）
                    datamodel.SetTag(j, (int)SpreadColKeys.SagyoBashoCode, wk_info);
                }
        ...
        private List<SagyoBashoInfo> GetMatchedList(IList<SagyoBashoInfo> list)
        {
            List<SagyoBashoInfo> rt_list = new List<SagyoBashoInfo>();
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<SagyoBashoInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (SagyoBashoInfo item in wk_list)
            {
                //「コード」+「名称」+「カナ」であいまい検索
                string item_mei =
                    item.SagyoBashoCode.Trim() + Environment.NewLine
        ...
        /// <summary>
        /// 選択している作業場所情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<SagyoBashoInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// <summary>
        /// 一覧にて選択中の作業場所情報を取得します。
        /// </summary>
        /// <returns>作業場所情報</returns>
        private List<SagyoBashoInfo> GetInfoByListOnSelection()
        {
            List<SagyoBashoInfo> rt_list = new List<SagyoBashoInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        SagyoBashoInfo info =
                            ((SagyoBashoInfo)sheet0.Cells[item.Row + i, (int)SpreadColKeys.SagyoBashoCode].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public SagyoBashoInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された作業場所情報を一覧で取得します。
        /// </summary>
        public List<SagyoBashoInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchSagyoBashoFrame();
        }
        ...
        private void CmnSearchSagyoBashoFrame_KeyDown(object sender, KeyEventArgs e)
        {
