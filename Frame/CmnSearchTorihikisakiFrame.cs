...
{
    public partial class CmnSearchTorihikisakiFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "取引先検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Torihikisaki searchTblClass;

        /// <summary>
        /// 取引先情報のリストを保持する領域
        /// </summary>
        private IList<TorihikisakiInfo> TorihikisakiList = null;
        ...
        /// 傭車データを表示するかどうかの値を保持
        /// </summary>
        private bool _showYoshaFlag = false;

        /// <summary>
        /// 傭車以外のデータを表示するかどうかの値を保持
        /// </summary>
        private bool _showWithoutYoshaFlag = false;
        ...
        /// 略称列番号
        /// </summary>
        private const int COL_SNAME = 2;
        /// <summary>
        /// カナ列番号
        /// </summary>
        private const int COL_KANA = 3;
        /// <summary>
        /// 使用停止列番号
        /// </summary>
        private const int COL_MUKO = 4;
        /// <summary>
        /// 検索一覧最大列数
        /// </summary>
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public CmnSearchTorihikisakiFrame()
        {
            InitializeComponent();
        }
        ...
        private void InitCmnSearchTorihikisakiFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new Torihikisaki();
        ...
        /// メニュー関連を初期化します。
        /// </summary>
        private void InitMenuTorihikisaki()
        {
            //操作メニュー
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtTorihikisakiMei, ctl => ctl.Text, this.edtTorihikisakiMei_Validating));
        }
        ...
        private void ClearInputs()
        {
            ...
            this.chkYoshaFlag.Checked = _showYoshaFlag;
            this.chkWithoutYoshaFlag.Checked = _showWithoutYoshaFlag;
        ...
        {
            //取引先一覧を取得
            this.TorihikisakiList =
                this.searchTblClass.GetList(new TorihikisakiSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.TorihikisakiList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<TorihikisakiInfo> wk_list =
                    this.GetMatchedList(this.TorihikisakiList).OrderBy(x => x.ToraDONTorihikiCode).ToList();
                ...
                {
                    TorihikisakiInfo wk_info = wk_list[j];
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.ToraDONTorihikiCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.ToraDONTorihikiName);
                    datamodel.SetValue(j, COL_SNAME, wk_info.ToraDONTorihikiShortName);
                    datamodel.SetValue(j, COL_KANA, wk_info.ToraDONTorihikiNameKana);
        ...
        private List<TorihikisakiInfo> GetMatchedList(IList<TorihikisakiInfo> list)
        {
            ...
            //傭車表示フラグによってデータ再抽出
            IList<TorihikisakiInfo> wk_list =
                this.chkYoshaFlag.Checked
                    ? list
                    : list.Where(x => x.CharterKbn != (int)DefaultProperty.YoshaKbn.Yosha).ToList();

            //傭車以外表示フラグによってデータ再抽出
            wk_list =
                this.chkWithoutYoshaFlag.Checked
                    ? wk_list
                    : wk_list.Where(x => x.CharterKbn != (int)DefaultProperty.YoshaKbn.Torihikisaki).ToList();

            //使用停止表示フラグによってデータ再抽出
            wk_list =
                this.chkAllFlag.Checked ? wk_list : wk_list.Where(x => !x.ToraDONDisableFlag).ToList();

            foreach (TorihikisakiInfo item in wk_list)
            {
                //「コード」+「名称」+「略称」+「カナ」であいまい検索
                string item_mei =
                    Convert.ToInt32(item.ToraDONTorihikiCode) + Environment.NewLine
                    + item.ToraDONTorihikiName.Trim() + Environment.NewLine
                    + item.ToraDONTorihikiShortName.Trim() + Environment.NewLine
                    + item.ToraDONTorihikiNameKana.Trim() + Environment.NewLine;
        ...
        /// <summary>
        /// 選択している取引先情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<TorihikisakiInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        private List<TorihikisakiInfo> GetInfoByListOnSelection()
        {
            List<TorihikisakiInfo> rt_list = new List<TorihikisakiInfo>();
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    for (int i = 0; i < item.RowCount; i++)
                    {
                        TorihikisakiInfo info =
                            ((TorihikisakiInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public TorihikisakiInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された取引先情報を一覧で取得します。
        /// </summary>
        public List<TorihikisakiInfo> SelectedInfoList { get; private set; }
        ...
        /// <summary>
        /// 傭車データを表示するかどうかの値を取得・設定します（True:表示する、False:表示しない）
        /// </summary>
        public bool ShowYoshaFlag
        {
            get { return this._showYoshaFlag; }
            set { this._showYoshaFlag = value; }
        }

        /// <summary>
        /// 傭車以外のデータを表示するかどうかの値を取得・設定します（True:表示する、False:表示しない）
        /// </summary>
        public bool ShowWithoutYoshaFlag
        {
            get { return this._showWithoutYoshaFlag; }
            set { this._showWithoutYoshaFlag = value; }
        }
        ...
        {
            this.InitCmnSearchTorihikisakiFrame();
        }
        ...
        private void CmnSearchTorihikisakiFrame_KeyDown(object sender, KeyEventArgs e)
        {
