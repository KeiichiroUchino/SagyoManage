...
{
    public partial class CmnSearchPointGroupFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "発着地グループ検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private PointGroup searchTblClass;

        /// <summary>
        /// 発着地グループ情報のリストを保持する領域
        /// </summary>
        private IList<PointGroupInfo> PointGroupList = null;
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public CmnSearchPointGroupFrame()
        {
            InitializeComponent();
        }
        ...
        private void InitCmnSearchPointGroupFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new PointGroup();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtItemGroupMei, ctl => ctl.Text, this.edtItemGroupMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //発着地グループ一覧を取得
            this.PointGroupList =
                this.searchTblClass.GetList(new PointGroupSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.PointGroupList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<PointGroupInfo> wk_list =
                    this.GetMatchedList(this.PointGroupList).OrderBy(x => x.PointGroupCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.PointGroupCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.PointGroupName);
                    datamodel.SetValue(j, COL_KANA, wk_info.PointGroupNameKana);
        ...
        private List<PointGroupInfo> GetMatchedList(IList<PointGroupInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<PointGroupInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (PointGroupInfo item in wk_list)
            {
                //「コード」+「名称」+「カナ」であいまい検索
                string item_mei =
        ...
        /// <summary>
        /// 選択している発着地グループ情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<PointGroupInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// <summary>
        /// 一覧にて選択中の発着地グループ情報を取得します。
        /// </summary>
        /// <returns>発着地グループ情報</returns>
        private List<PointGroupInfo> GetInfoByListOnSelection()
        {
            List<PointGroupInfo> rt_list = new List<PointGroupInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        PointGroupInfo info =
                            ((PointGroupInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public PointGroupInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された発着地グループ情報を一覧で取得します。
        /// </summary>
        public List<PointGroupInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchPointGroupFrame();
        }
        ...
        private void CmnSearchPointGroupFrame_KeyDown(object sender, KeyEventArgs e)
        {
