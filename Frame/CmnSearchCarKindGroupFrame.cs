...
{
    public partial class CmnSearchCarKindGroupFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "車種グループ検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private CarKindGroup searchTblClass;

        /// <summary>
        /// 車種グループ情報のリストを保持する領域
        /// </summary>
        private IList<CarKindGroupInfo> CarKindGroupList = null;
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public CmnSearchCarKindGroupFrame()
        {
            InitializeComponent();
        }
        ...
        private void InitCmnSearchCarKindGroupFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new CarKindGroup();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtCarKindGroupMei, ctl => ctl.Text, this.edtCarKindGroupMei_Validating));
        }
        ...
        {
            //車種グループ一覧を取得
            this.CarKindGroupList =
                this.searchTblClass.GetList(new CarKindGroupSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.CarKindGroupList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<CarKindGroupInfo> wk_list =
                    this.GetMatchedList(this.CarKindGroupList).OrderBy(x => x.CarKindGroupCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.CarKindGroupCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.CarKindGroupName);
        ...
        private List<CarKindGroupInfo> GetMatchedList(IList<CarKindGroupInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<CarKindGroupInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (CarKindGroupInfo item in wk_list)
            {
                //「コード」+「名称」であいまい検索
                string item_mei =
        ...
        /// <summary>
        /// 選択している車種グループ情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<CarKindGroupInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// <summary>
        /// 一覧にて選択中の車種グループ情報を取得します。
        /// </summary>
        /// <returns>車種グループ情報</returns>
        private List<CarKindGroupInfo> GetInfoByListOnSelection()
        {
            List<CarKindGroupInfo> rt_list = new List<CarKindGroupInfo>();
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    for (int i = 0; i < item.RowCount; i++)
                    {
                        CarKindGroupInfo info =
                            ((CarKindGroupInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public CarKindGroupInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された車種グループ情報を一覧で取得します。
        /// </summary>
        public List<CarKindGroupInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchCarKindGroupFrame();
        }
        ...
        private void CmnSearchCarKindGroupFrame_KeyDown(object sender, KeyEventArgs e)
        {
