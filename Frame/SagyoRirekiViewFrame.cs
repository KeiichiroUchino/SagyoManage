...
{
    public partial class SagyoRirekiViewFrame : Form, IFrameBase
    {
        #region コンストラクタ
        ...
        /// 画面のタイトル
        /// </summary>
        private const string WINDOW_TITLE = "作業履歴照会";
        ...
        private void ClearSearchInputs()
        {
            ...
            this.dteSagyoYMDFrom.Value = DateTime.Today.Date.AddMonths(-1);
            this.dteSagyoYMDTo.Value = DateTime.Today.Date.AddMonths(1);
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            validatingEventRaiserCollection.Add(
                ControlValidatingEventRaiser.Create(this.dteSagyoYMDTo, ctl => ctl.Text, this.dteSagyoYMDTo_Validating));
            validatingEventRaiserCollection.Add(
        ...
        private SagyoAnkenSearchParameter GetSearchParameter()
        {
            ...
            para.SearchKbn = (int)SagyoAnken.SearchKbnEnum.History;

            // 作業日（From）
            if (this.dteSagyoYMDFrom.Value != null)
            {
                para.SagyoStartDateTime = new DateTime(
                    this.dteSagyoYMDFrom.Value.Value.Year
                    , this.dteSagyoYMDFrom.Value.Value.Month
                    , this.dteSagyoYMDFrom.Value.Value.Day
                    , 0
                    , 0
                    , 0);
            }
            ...
            {
                para.SagyoStartDateTime = DateTime.MinValue;
            }

            // 作業日（To）
            if (this.dteSagyoYMDTo.Value != null)
            {
                para.SagyoEndDateTime = new DateTime(
                    this.dteSagyoYMDTo.Value.Value.Year
                    , this.dteSagyoYMDTo.Value.Value.Month
                    , this.dteSagyoYMDTo.Value.Value.Day
                    , 23
                    , 59
                    , 59);
            }
            ...
            {
                para.SagyoEndDateTime = DateTime.MaxValue;
            }
        ...
        private void SetJuchuListMrow(decimal id)
        {
            ...
            var list = this._SagyoAnkenInfoList
            ...
            try
            {
                ...
                foreach (var item in list)
                {
                    ...
                    datamodel.SetValue(row, (int)SpreadColKeys.StaffSu, item.StaffSu);
                    datamodel.SetValue(row, (int)SpreadColKeys.CarSu, item.CarSu);
        ...
        private bool CheckInputs()
        {
            ...
            //コードの必須チェック
            if (rt_val && string.IsNullOrWhiteSpace(this.edtKeiyakuCd.Text))
            {
                rt_val = false;
                msg = FrameUtilites.GetDefineMessage(
                        "MW2203004", new string[] { "コード" });

                ctl = this.edtKeiyakuCd;
            }
        ...
        private void DoGetData(decimal id = 0)
        {
            try
            {
                ...
                if (this.ValidateChildren(ValidationConstraints.None) && this.CheckInputs())
                {
                    ...
                    // 画面コントローラに取得を指示
                    // 契約一覧取得
                    this._SagyoAnkenInfoList = this._SagyoAnken.GetListInternal(null, para).ToList();
        ...
        /// </summary>
        /// <param name="e"></param>
        private void dteSagyoYMDTo_Validating(object sender, CancelEventArgs e)
        {
            this.InitSheet();
        }
