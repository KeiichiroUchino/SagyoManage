...
{
    public partial class CmnSearchPointFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "発着地検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Jpsys.HaishaManageV10.SQLServerDAL.Point searchTblClass;

        /// <summary>
        /// 発着地情報のリストを保持する領域
        /// </summary>
        private IList<PointInfo> PointList = null;
        ...
        private void InitCmnSearchPointFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new Jpsys.HaishaManageV10.SQLServerDAL.Point();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtPointMei, ctl => ctl.Text, this.edtPointMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //発着地一覧を取得
            this.PointList =
                this.searchTblClass.GetList(new PointSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                //抽出後のリストを取得（同時に並べ替え）
                List<PointInfo> wk_list =
                    this.GetMatchedList(this.PointList).OrderBy(x => x.ToraDONPointCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.ToraDONPointCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.ToraDONPointName);
                    datamodel.SetValue(j, COL_KANA, wk_info.ToraDONPointNameKana);
                    datamodel.SetValue(j, COL_HOMEN, wk_info.HomenName);
        ...
        /// <summary>
        /// 選択している発着地情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<PointInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// 一覧にて選択中の発着地情報を取得します。
        /// </summary>
        /// <returns>発着地情報</returns>
        private List<PointInfo> GetInfoByListOnSelection()
        {
            List<PointInfo> rt_list = new List<PointInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        PointInfo info =
                            ((PointInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public PointInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された発着地情報を一覧で取得します。
        /// </summary>
        public List<PointInfo> SelectedInfoList { get; private set; }
        ...
        private void CmnSearchPointFrame_KeyDown(object sender, KeyEventArgs e)
        {
