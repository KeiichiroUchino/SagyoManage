...
{
    public partial class CmnSearchCarFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "車両検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Car searchTblClass;

        /// <summary>
        /// 車両情報のリストを保持する領域
        /// </summary>
        private IList<CarInfo> CarList = null;
        ...
        /// 車番列番号
        /// </summary>
        private const int COL_SHABAN = 1;
        /// <summary>
        /// 陸運局列番号
        /// </summary>
        private const int COL_CARNAME = 2;
        /// <summary>
        /// 使用停止列番号
        /// </summary>
        ...
        private void InitCmnSearchCarFrame()
        {
            //画面タイトルの設定
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtCarMei, ctl => ctl.Text, this.edtCarMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //車両一覧を取得
            this.CarList =
                this.searchTblClass.GetList(new CarSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                //抽出後のリストを取得（同時に並べ替え）
                List<CarInfo> wk_list =
                    this.GetMatchedList(this.CarList)
                    .OrderBy(x => x.CarCode)
                    .ThenBy(x => x.LicPlateCarNo).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.CarCode);
                    datamodel.SetValue(j, COL_SHABAN, wk_info.LicPlateCarNo);
                    datamodel.SetValue(j, COL_CARNAME, wk_info.CarName);
        ...
        private List<CarInfo> GetMatchedList(IList<CarInfo> list)
        {
            ...
            foreach (CarInfo item in wk_list)
            {
                //「コード」+「車番」+「車両名称」であいまい検索
                string item_mei =
                      Convert.ToInt32(item.CarCode).ToString() + Environment.NewLine
        ...
        /// <summary>
        /// 選択している車両情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<CarInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        private List<CarInfo> GetInfoByListOnSelection()
        {
            List<CarInfo> rt_list = new List<CarInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        CarInfo info =
                            ((CarInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 検索結果をすべて選択します。
        /// </summary>
        private void SelectAllListGrid()
        {
            this.fpListGrid.Sheets[0].ClearSelection();
            this.fpListGrid.Sheets[0].AddSelection(0, 0,
                fpListGrid.Sheets[0].RowCount, fpListGrid.Sheets[0].ColumnCount);
        }

        /// <summary>
        /// 検索結果の選択を解除します。（選択してあるものは全て解除します）
        /// </summary>
        private void CancelSelectedListGrid()
        {
            //選択を解除して１行目を選択
            this.fpListGrid.Focus();
            this.fpListGrid.Sheets[0].ClearSelection();
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public CarInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された車両情報を一覧で取得します。
        /// </summary>
        public List<CarInfo> SelectedInfoList { get; private set; }
        ...
        private void CmnSearchCarFrame_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void cmbCarKbn_SelectedValueChanged(object sender, EventArgs e)
        {
            // 検索処理
            this.SetListSheet();
        }
