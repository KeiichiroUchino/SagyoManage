...
{
    public partial class SagyoBashoViewFrame : Form, IFrameBase
    {
        #region コンストラクタ
        ...
        /// 画面のタイトル
        /// </summary>
        private const string WINDOW_TITLE = "作業場所一覧";
        ...
        /// <summary>
        /// 検索した作業場所情報リストを保持する領域
        /// </summary>
        private List<SagyoBashoInfo> _SagyoBashoInfoList = new List<SagyoBashoInfo>();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            validatingEventRaiserCollection.Add(
                   ControlValidatingEventRaiser.Create(this.edtSagyoBashoCode, ctl => ctl.Text, this.edtSagyoBashoCode_Validating));
            validatingEventRaiserCollection.Add(
                   ControlValidatingEventRaiser.Create(this.edtSagyoBashoName, ctl => ctl.Text, this.edtSagyoBashoName_Validating));
            validatingEventRaiserCollection.Add(
        ...
        /// 作業場所情報一覧の検索条件を取得します
        /// </summary>
        /// <returns>作業場所情報一覧の検索条件</returns>
        private SagyoBashoSearchParameter GetSearchParameter()
        {
            ...
            //作業場所コード
            para.SagyoBashoCodeAmbiguous = this.edtSagyoBashoCode.Text;
            //作業場所名
            para.SagyoBashoName = this.edtSagyoBashoName.Text;
        ...
        private void SetListMrow(decimal id)
        {
            ...
            var list = this._SagyoBashoInfoList
                        .OrderBy(x => x.SagyoBashoCode)
                        .ToList();
            ...
            try
            {
                ...
                foreach (var item in wk_list)
                {
                    ...
                    //画面に値を設定していく
                    if (item.SagyoBashoId == id && id != 0)
                    {
                        selectrowidx = row;
                    }

                    datamodel.SetValue(row, (int)SpreadColKeys.SagyoBashoId, item.SagyoBashoId);
                    ...
                    datamodel.SetValue(row, (int)SpreadColKeys.SagyoBashoAddress1, item.SagyoBashoAddress1);
                    datamodel.SetValue(row, (int)SpreadColKeys.SagyoBashoAddress2, item.SagyoBashoAddress2);
                    ...
                    //タグに情報を保持しておく
                    datamodel.SetTag(row, (int)SpreadColKeys.SagyoBashoId, item);
        ...
        private void DoGetData(decimal id = 0)
        {
            try
            {
                ...
                {
                    //作業場所情報検索条件取得
                    SagyoBashoSearchParameter para = this.GetSearchParameter();

                    // 画面コントローラに取得を指示
                    // 作業場所一覧取得
                    this._SagyoBashoInfoList = this._SagyoBasho.GetList(para).ToList();

                    // 0件なら処理しない
                    if (this._SagyoBashoInfoList.Count == 0 && this._ReloadMessageFlg)
                    {
                        MessageBox.Show(
        ...
        private void ShowMitsumoriFrame(decimal id = 0)
        {
            ...
            SagyoBashoTorokuFrame f = null;

            if (id.CompareTo(decimal.Zero) > 0)
            {
                f = new SagyoBashoTorokuFrame(id);
            }
            else
            {
                f = new SagyoBashoTorokuFrame();
            }

            f.InitFrame();
            f.SagyoBashoIchiran = this;
            f.ShowDialog(this);
            ...
            if (currowidx >= 0)
            {
                ...
                rt_val =
                    ((decimal)sheet0.Models.Data.GetValue(
                        ...
                        (int)SpreadColKeys.SagyoBashoId));
            }
        ...
        private void setDtlFocus(decimal id, bool dtlFlg)
        {
            ...
            {
                var dtlId = Convert.ToDecimal(sheet.Models.Data.GetValue(sheet.GetModelRowFromViewRow(i), (int)SpreadColKeys.SagyoBashoId));
        ...
        /// </summary>
        /// <param name="e"></param>
        private void edtSagyoBashoCode_Validating(object sender, CancelEventArgs e)
        {
            this.InitSheet();
        }
        ...
        /// </summary>
        /// <param name="e"></param>
        private void edtSagyoBashoName_Validating(object sender, CancelEventArgs e)
        {
            this.InitSheet();
        }
