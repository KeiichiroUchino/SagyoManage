...
{
    public partial class CmnSearchClmClassFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "請求部門検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private ToraDONClmClass searchTblClass;

        /// <summary>
        /// 請求部門情報のリストを保持する領域
        /// </summary>
        private IList<ToraDONClmClassInfo> ClmClassList = null;
        ...
        private void InitCmnSearchClmClassFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new ToraDONClmClass();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtClmClassMei, ctl => ctl.Text, this.edtClmClassMei_Validating));
            validatingEventRaiserCollection.Add(
    ControlValidatingEventRaiser.Create(this.numSearchTokuisakiCode, ctl => ctl.Text, this.numSearchTokuisakiCode_Validating));
        ...
        private void DoGetData()
        {
            //請求部門一覧を取得
            this.ClmClassList =
                this.searchTblClass.GetList(new ToraDONClmClassSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.ClmClassList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<ToraDONClmClassInfo> wk_list =
                    this.GetMatchedList(this.ClmClassList).OrderBy(x => x.ClmClassCode).ToList();
                ...
                {
                    ToraDONClmClassInfo wk_info = wk_list[j];
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.ClmClassCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.ClmClassName);
                    datamodel.SetValue(j, COL_SNAME, wk_info.ClmClassShortName);
                    datamodel.SetValue(j, COL_KANA, wk_info.ClmClassNameKana);
        ...
        private List<ToraDONClmClassInfo> GetMatchedList(IList<ToraDONClmClassInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<ToraDONClmClassInfo> wk_list =
            ...
            foreach (ToraDONClmClassInfo item in wk_list)
            {
                ...
                if (0 == tokuisakiid || tokuisakiid == item.TokuisakiId)
                {
                    ...
                    //「コード」+「名称」+「カナ」であいまい検索
                    string item_mei =
                        Convert.ToInt32(item.ClmClassCode) + Environment.NewLine
                        + item.ClmClassName.Trim() + Environment.NewLine
                        + item.ClmClassNameKana.Trim() + Environment.NewLine;
        ...
        /// <summary>
        /// 選択している請求部門情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<ToraDONClmClassInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// <summary>
        /// 一覧にて選択中の請求部門情報を取得します。
        /// </summary>
        /// <returns>請求部門情報</returns>
        private List<ToraDONClmClassInfo> GetInfoByListOnSelection()
        {
            List<ToraDONClmClassInfo> rt_list = new List<ToraDONClmClassInfo>();
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    for (int i = 0; i < item.RowCount; i++)
                    {
                        ToraDONClmClassInfo info =
                            ((ToraDONClmClassInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public ToraDONClmClassInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された請求部門情報を一覧で取得します。
        /// </summary>
        public List<ToraDONClmClassInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchClmClassFrame();
        }
        ...
        private void CmnSearchClmClassFrame_KeyDown(object sender, KeyEventArgs e)
        {
