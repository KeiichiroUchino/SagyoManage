...
{
    public partial class CmnSearchMeishoFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "検索";

        /// <summary>
        /// スプレッドの名称列のタイトル
        /// </summary>
        private const string SPRED_NAME_HEADER_TITLE = "名";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Meisho searchTblClass;

        /// <summary>
        /// 名称情報のリストを保持する領域
        /// </summary>
        private IList<MeishoInfo> MeishoList = null;
        ...
        /// <summary>
        /// コンストラクタの引数で指定される名称区分を保持
        /// </summary>
        private DefaultProperty.MeishoKbn _nameKbn = 0;
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public CmnSearchMeishoFrame()
        {
            InitializeComponent();
        }

        /// <summary>
        /// 名称の区分を指定して、本クラスのインスタンスを初期化します。
        /// </summary>
        /// <param name="nameKbn">名称区分</param>
        public CmnSearchMeishoFrame(DefaultProperty.MeishoKbn nameKbn)
        {
            InitializeComponent();

            this._nameKbn = nameKbn;
        }
        ...
        private void InitCmnSearchMeishoFrame()
        {
            //画面タイトルの設定
            this.Text = DefaultProperty.GetMeishoKbnString(this._nameKbn) + WINDOW_TITLE;

            //未入力時の表示内容
            this.edtSearchText.AlternateText.DisplayNull.Text = DefaultProperty.GetMeishoKbnString(this._nameKbn) + "の検索";
            this.edtSearchText.AlternateText.Null.Text = DefaultProperty.GetMeishoKbnString(this._nameKbn) + "の検索";

            //名称列のタイトル
            this.fpListGrid.ActiveSheet.ColumnHeader.Cells[0, COL_NAME].Text =
                DefaultProperty.GetMeishoKbnString(this._nameKbn) + SPRED_NAME_HEADER_TITLE;
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new Meisho();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtItemMei, ctl => ctl.Text, this.edtItemMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //名称一覧を取得
            this.MeishoList =
                this.searchTblClass.GetList(new MeishoSearchParameter() {
                MeishoKbn = (int)this._nameKbn});
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.MeishoList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<MeishoInfo> wk_list =
                    this.GetMatchedList(this.MeishoList).OrderBy(x => x.MeishoCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.MeishoCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.Meisho);
                    datamodel.SetValue(j, COL_KANA, wk_info.MeishoKana);
        ...
        private List<MeishoInfo> GetMatchedList(IList<MeishoInfo> list)
        {
            ...
            foreach (MeishoInfo Meisho in wk_list)
            {
                //「コード」+「名称」+「カナ」であいまい検索
                string Meisho_mei =
                    Convert.ToInt32(Meisho.MeishoCode) + Environment.NewLine
                    + Meisho.Meisho.Trim() + Environment.NewLine
                    + Meisho.MeishoKana.Trim() + Environment.NewLine;

                //半角を全角に変換する
                string zenkaku =
                    Microsoft.VisualBasic.Strings.StrConv(
                        Meisho_mei, Microsoft.VisualBasic.VbStrConv.Wide, 0x411);
        ...
        /// <summary>
        /// 選択している名称情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<MeishoInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        private List<MeishoInfo> GetInfoByListOnSelection()
        {
            List<MeishoInfo> rt_list = new List<MeishoInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        MeishoInfo info =
                            ((MeishoInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public MeishoInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された名称情報を一覧で取得します。
        /// </summary>
        public List<MeishoInfo> SelectedInfoList { get; private set; }
        ...
        private void CmnSearchMeishoFrame_KeyDown(object sender, KeyEventArgs e)
        {
