...
{
    public partial class CmnSearchOperatorFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "利用者検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Operator searchTblClass;

        /// <summary>
        /// 利用者情報のリストを保持する領域
        /// </summary>
        private IList<OperatorInfo> operatorList = null;
        ...
        private void InitCmnSearchOperatorFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new Operator();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtOperatorMei, ctl => ctl.Text, this.edtOperatorMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //利用者一覧を取得
            this.operatorList =
                this.searchTblClass.GetList(new OperatorSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                //抽出後のリストを取得（同時に並べ替え）
                List<OperatorInfo> wk_list =
                    this.GetMatchedList(this.operatorList).OrderBy(x => x.OperatorCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.OperatorCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.OperatorName);
        ...
        private List<OperatorInfo> GetMatchedList(IList<OperatorInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<OperatorInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (OperatorInfo item in wk_list)
            {
                //「コード」+「名称」であいまい検索
                string item_mei =
        ...
        /// <summary>
        /// 選択している利用者情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<OperatorInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        private List<OperatorInfo> GetInfoByListOnSelection()
        {
            List<OperatorInfo> rt_list = new List<OperatorInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        OperatorInfo info =
                            ((OperatorInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public OperatorInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された利用者情報を一覧で取得します。
        /// </summary>
        public List<OperatorInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchOperatorFrame();
        }
        ...
        private void CmnSearchOperatorFrame_KeyDown(object sender, KeyEventArgs e)
        {
