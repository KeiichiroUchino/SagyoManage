...
{
    public partial class CmnSearchHomenGroupFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "方面グループ検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private HomenGroup searchTblClass;

        /// <summary>
        /// 方面グループ情報のリストを保持する領域
        /// </summary>
        private IList<HomenGroupInfo> HomenGroupList = null;
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public CmnSearchHomenGroupFrame()
        {
            InitializeComponent();
        }
        ...
        private void InitCmnSearchHomenGroupFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new HomenGroup();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtHomenGroupMei, ctl => ctl.Text, this.edtHomenGroupMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //方面グループ一覧を取得
            this.HomenGroupList =
                this.searchTblClass.GetList(new HomenGroupSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.HomenGroupList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<HomenGroupInfo> wk_list =
                    this.GetMatchedList(this.HomenGroupList).OrderBy(x => x.HomenGroupCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.HomenGroupCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.HomenGroupName);
        ...
        private List<HomenGroupInfo> GetMatchedList(IList<HomenGroupInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<HomenGroupInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (HomenGroupInfo item in wk_list)
            {
                //「コード」+「名称」であいまい検索
                string item_mei =
        ...
        /// <summary>
        /// 選択している方面グループ情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<HomenGroupInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// <summary>
        /// 一覧にて選択中の方面グループ情報を取得します。
        /// </summary>
        /// <returns>方面グループ情報</returns>
        private List<HomenGroupInfo> GetInfoByListOnSelection()
        {
            List<HomenGroupInfo> rt_list = new List<HomenGroupInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        HomenGroupInfo info =
                            ((HomenGroupInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public HomenGroupInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された方面グループ情報を一覧で取得します。
        /// </summary>
        public List<HomenGroupInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchHomenGroupFrame();
        }
        ...
        private void CmnSearchHomenGroupFrame_KeyDown(object sender, KeyEventArgs e)
        {
