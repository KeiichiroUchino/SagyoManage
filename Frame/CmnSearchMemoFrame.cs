...
{
    public partial class CmnSearchMemoFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "備考検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Memo searchTblClass;

        /// <summary>
        /// 備考情報のリストを保持する領域
        /// </summary>
        private IList<ToraDONMemoInfo> MemoList = null;
        ...
        private void InitCmnSearchMemoFrame()
        {
            //画面タイトルの設定
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtMemoMei, ctl => ctl.Text, this.edtMemoMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //備考一覧を取得
            this.MemoList =
                this.searchTblClass.GetList(new MemoSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                //抽出後のリストを取得（同時に並べ替え）
                List<ToraDONMemoInfo> wk_list =
                    this.GetMatchedList(this.MemoList).OrderBy(x => x.ToraDONMemoCode).ToList();
                ...
                {
                    ToraDONMemoInfo wk_info = wk_list[j];
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.ToraDONMemoCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.ToraDONMemoName);
        ...
        private List<ToraDONMemoInfo> GetMatchedList(IList<ToraDONMemoInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<ToraDONMemoInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (ToraDONMemoInfo item in wk_list)
            {
                //「コード」+「名称」であいまい検索
                string item_mei =
                    Convert.ToInt32(item.ToraDONMemoCode) + Environment.NewLine
                    + item.ToraDONMemoName.Trim() + Environment.NewLine;
        ...
        /// <summary>
        /// 選択している備考情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<ToraDONMemoInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// 一覧にて選択中の備考情報を取得します。
        /// </summary>
        /// <returns>備考情報</returns>
        private List<ToraDONMemoInfo> GetInfoByListOnSelection()
        {
            List<ToraDONMemoInfo> rt_list = new List<ToraDONMemoInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        ToraDONMemoInfo info =
                            ((ToraDONMemoInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public ToraDONMemoInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された備考情報を一覧で取得します。
        /// </summary>
        public List<ToraDONMemoInfo> SelectedInfoList { get; private set; }
        ...
        private void CmnSearchMemoFrame_KeyDown(object sender, KeyEventArgs e)
        {
