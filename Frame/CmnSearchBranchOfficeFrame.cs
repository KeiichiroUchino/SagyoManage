...
{
    public partial class CmnSearchBranchOfficeFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "営業所検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private ToraDONBranchOffice searchTblClass;

        /// <summary>
        /// 営業所情報のリストを保持する領域
        /// </summary>
        private IList<ToraDONBranchOfficeInfo> BranchOfficeList = null;
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public CmnSearchBranchOfficeFrame()
        {
            InitializeComponent();
        }
        ...
        private void InitCmnSearchBranchOfficeFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new ToraDONBranchOffice();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtBranchOfficeMei, ctl => ctl.Text, this.edtBranchOfficeMei_Validating));
        }
        ...
        private void DoGetData()
        {
            ...
            this.BranchOfficeList =
                this.searchTblClass.GetList(new ToraDONBranchOfficeSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                //抽出後のリストを取得（同時に並べ替え）
                List<ToraDONBranchOfficeInfo> wk_list =
                    this.GetMatchedList(this.BranchOfficeList).OrderBy(x => x.BranchOfficeCode).ToList();
                ...
                {
                    ToraDONBranchOfficeInfo wk_info = wk_list[j];
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.BranchOfficeCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.BranchOfficeName);
                    datamodel.SetValue(j, COL_SNAME, wk_info.BranchOfficeShortName);
        ...
        private List<ToraDONBranchOfficeInfo> GetMatchedList(IList<ToraDONBranchOfficeInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<ToraDONBranchOfficeInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (ToraDONBranchOfficeInfo item in wk_list)
            {
                //「コード」+「名称」+「略称」であいまい検索
                string item_mei =
                    Convert.ToInt32(item.BranchOfficeCode) + Environment.NewLine
                    + item.BranchOfficeName.Trim() + Environment.NewLine
                    + item.BranchOfficeShortName.Trim() + Environment.NewLine;
        ...
        /// <summary>
        /// 選択している営業所情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<ToraDONBranchOfficeInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// 一覧にて選択中の営業所情報を取得します。
        /// </summary>
        /// <returns>営業所情報</returns>
        private List<ToraDONBranchOfficeInfo> GetInfoByListOnSelection()
        {
            List<ToraDONBranchOfficeInfo> rt_list = new List<ToraDONBranchOfficeInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        ToraDONBranchOfficeInfo info =
                            ((ToraDONBranchOfficeInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public ToraDONBranchOfficeInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された営業所情報を一覧で取得します。
        /// </summary>
        public List<ToraDONBranchOfficeInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchBranchOfficeFrame();
        }
        ...
        private void CmnSearchBranchOfficeFrame_KeyDown(object sender, KeyEventArgs e)
        {
