...
{
    public partial class CmnSearchCarKindFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "車種検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Jpsys.HaishaManageV10.SQLServerDAL.CarKind searchTblClass;

        /// <summary>
        /// 車種情報のリストを保持する領域
        /// </summary>
        private IList<CarKindInfo> CarKindList = null;
        ...
        private void InitCmnSearchCarKindFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new Jpsys.HaishaManageV10.SQLServerDAL.CarKind();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtCarKindMei, ctl => ctl.Text, this.edtCarKindMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //車種一覧を取得
            this.CarKindList =
                this.searchTblClass.GetList(new CarKindSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.CarKindList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<CarKindInfo> wk_list =
                    this.GetMatchedList(this.CarKindList).OrderBy(x => x.ToraDONCarKindCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.ToraDONCarKindCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.ToraDONCarKindName);
                    datamodel.SetValue(j, COL_SNAME, wk_info.ToraDONCarKindSNM);
                    datamodel.SetValue(j, COL_KANA, wk_info.ToraDONCarKindNameKana);
        ...
        private List<CarKindInfo> GetMatchedList(IList<CarKindInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<CarKindInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.ToraDONDisableFlag).ToList();

            foreach (CarKindInfo item in wk_list)
            {
                ...
                string item_mei =
                    ...
                    + item.ToraDONCarKindSNM.Trim() + Environment.NewLine
        ...
        /// <summary>
        /// 選択している車種情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<CarKindInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        private List<CarKindInfo> GetInfoByListOnSelection()
        {
            List<CarKindInfo> rt_list = new List<CarKindInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        CarKindInfo info =
                            ((CarKindInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public CarKindInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された車種情報を一覧で取得します。
        /// </summary>
        public List<CarKindInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchCarKindFrame();
        }
        ...
        private void CmnSearchCarKindFrame_KeyDown(object sender, KeyEventArgs e)
        {
