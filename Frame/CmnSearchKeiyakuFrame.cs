...
{
    public partial class CmnSearchKeiyakuFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "契約検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Jpsys.SagyoManage.SQLServerDAL.Keiyaku searchTblClass;

        /// <summary>
        /// 契約情報のリストを保持する領域
        /// </summary>
        private IList<KeiyakuInfo> KeiyakuList = null;
        ...
        ///// 契約開始日列番号
        ///// </summary>
        //private const int COL_STARTYMD = 1;
        ///// <summary>
        ///// 契約終了日列番号
        ///// </summary>
        //private const int COL_ENDYMD = 2;
        ...
        ///// 得意先名称列番号
        ///// </summary>
        //private const int COL_TOKUI_NAME = 4;
        ///// <summary>
        ///// 作業場所名称列番号
        ///// </summary>
        //private const int COL_SAGYOBASHO_NAME = 5;
        ///// <summary>
        ///// 作業大分類列番号
        ///// </summary>
        //private const int COL_SAGYO_DAI = 6;
        ///// <summary>
        ///// 作業中分類列番号
        ///// </summary>
        //private const int COL_SAGYO_CHU = 7;
        ///// <summary>
        ///// 作業小分類列番号
        ///// </summary>
        //private const int COL_SAGYO_SHO = 8;
        ///// <summary>
        ///// 使用停止列番号
        ///// </summary>
        //private const int COL_MUKO = 9;
        ///// <summary>
        ///// 検索一覧最大列数
        ///// </summary>
        //private const int COL_MAXCOLNUM_LIST = 10;
        ...
        private void InitCmnSearchKeiyakuFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new Jpsys.SagyoManage.SQLServerDAL.Keiyaku();
        ...
        private void InitListSheet()
        {
            ...
            for (int i = 0; i < COL_MAXCOLUM_LIST; i++)
            {
        ...
        /// メニュー関連を初期化します。
        /// </summary>
        private void InitMenuKeiyaku()
        {
            //操作メニュー
            this.InitActionMenuItem();
        }
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtKeiyakuMei, ctl => ctl.Text, this.edtKeiyakuMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //契約一覧を取得
            this.KeiyakuList =
                this.searchTblClass.GetList(new KeiyakuSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.KeiyakuList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<KeiyakuInfo> wk_list =
                    this.GetMatchedList(this.KeiyakuList).OrderBy(x => x.KeiyakuCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, (int)SpreadColKeys.KeiyakuCode, wk_info.KeiyakuCode);
                    datamodel.SetValue(j, (int)SpreadColKeys.KeiyakuName, wk_info.KeiyakuName);

                    if (wk_info.KeiyakuStartDate != DateTime.MinValue)
                    {
                        datamodel.SetValue(j, (int)SpreadColKeys.KeiyakuStartYMD, wk_info.KeiyakuStartDate.ToString("yyyy/MM/dd"));
                    }
                    else
                    {
                        datamodel.SetValue(j, (int)SpreadColKeys.KeiyakuStartYMD, null);
                    }

                    if (wk_info.KeiyakuEndDate != DateTime.MinValue)
                    {
                        datamodel.SetValue(j, (int)SpreadColKeys.KeiyakuEndYMD, wk_info.KeiyakuEndDate.ToString("yyyy/MM/dd"));
                    }
                    else
                    {
                        datamodel.SetValue(j, (int)SpreadColKeys.KeiyakuEndYMD, null);
                    }

                    datamodel.SetValue(j, (int)SpreadColKeys.TokuisakiCode, wk_info.TokuisakiCode);
                    datamodel.SetValue(j, (int)SpreadColKeys.TokuisakiName, wk_info.TokuisakiName);
                    datamodel.SetValue(j, (int)SpreadColKeys.SagyoBashoCode, wk_info.SagyoBashoCode);
                    datamodel.SetValue(j, (int)SpreadColKeys.SagyoBashoName, wk_info.SagyoBashoName);

                    datamodel.SetValue(j, (int)SpreadColKeys.SagyoDaiBunruiName, wk_info.SagyoDaiBunruiName);
                    datamodel.SetValue(j, (int)SpreadColKeys.SagyoChuBunruiName, wk_info.SagyoChuBunruiName);
                    datamodel.SetValue(j, (int)SpreadColKeys.SagyoShoBunruiName, wk_info.SagyoShoBunruiName);

                    datamodel.SetValue(j, (int)SpreadColKeys.ShiyoTeishi, wk_info.ShiyoTeishi);

                    // Tagへ1件分のデータを退避（後で使用）
                    datamodel.SetTag(j, (int)SpreadColKeys.KeiyakuCode, wk_info);
                }
        ...
        private List<KeiyakuInfo> GetMatchedList(IList<KeiyakuInfo> list)
        {
            List<KeiyakuInfo> rt_list = new List<KeiyakuInfo>();
            ...
            foreach (KeiyakuInfo item in wk_list)
            {
                //「コード」+「名称」+「カナ」であいまい検索
                string item_mei =
                    item.KeiyakuCode.Trim() + Environment.NewLine
                    + item.KeiyakuStartDate.ToString("yyyy/MM/dd") + Environment.NewLine
                    + item.KeiyakuEndDate.ToString("yyyy/MM/dd") + Environment.NewLine
                    + item.KeiyakuName.Trim() + Environment.NewLine
                    + item.TokuisakiName.Trim() + Environment.NewLine
                    + item.SagyoBashoName.Trim() + Environment.NewLine
                    + item.SagyoDaiBunruiName.Trim() + Environment.NewLine
                    + item.SagyoChuBunruiName.Trim() + Environment.NewLine
                    + item.SagyoShoBunruiName.Trim() + Environment.NewLine;
        ...
        /// <summary>
        /// 選択している契約情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<KeiyakuInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        private List<KeiyakuInfo> GetInfoByListOnSelection()
        {
            List<KeiyakuInfo> rt_list = new List<KeiyakuInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        KeiyakuInfo info =
                            ((KeiyakuInfo)sheet0.Cells[item.Row + i, (int)SpreadColKeys.KeiyakuCode].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public KeiyakuInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された契約情報を一覧で取得します。
        /// </summary>
        public List<KeiyakuInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchKeiyakuFrame();
        }
        ...
        private void CmnSearchKeiyakuFrame_KeyDown(object sender, KeyEventArgs e)
        {
