...
{
    public partial class HibetsuSagyoKeikakuhyouPrtToFrame : Form, IFrameBase
    {
        #region コンストラクタ
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public HibetsuSagyoKeikakuhyouPrtToFrame()
        {
            InitializeComponent();
        }
        ...
        /// 画面のタイトル
        /// </summary>
        private const string WINDOW_TITLE = "日別作業計画表";
        ...
        /// 印字用データ一覧を保持する領域
        /// </summary>
        private IList<HibetsuSagyoKeikakuhyouRptInfo> _RptList;

        /// <summary>
        /// 抽出情報を保持する領域
        /// </summary>
        private HibetsuSagyoKeikakuhyouRptInfoSearchParameter _ArgInfo;
        ...
        private void InitJuchuNyuryokuFrame()
        {
            ...
            //画面コントローラのインスタンスを作成
            this._HibetsuSagyoKeikakuhyouRpt = new HibetsuSagyoKeikakuhyouRpt();
        ...
        {
            this.dteSagyoYMDFrom.Value = DateTime.Today.Date.AddDays(1);
            this.dteSagyoYMDTo.Value = DateTime.Today.Date.AddDays(1);
            //メンバをクリア
        ...
        {
            //コントローラーにセット
            HibetsuSagyoKeikakuhyouRptInfoSearchParameter arginfo =
                new HibetsuSagyoKeikakuhyouRptInfoSearchParameter();

            // 作業日(開始)
            if (this.dteSagyoYMDFrom.Value != null)
            {
                arginfo.SagyoYMDFrom = this.dteSagyoYMDFrom.Value.Value;
            }
            else
            {
                arginfo.SagyoYMDFrom = DateTime.MinValue;
            }

            // 作業日(終了)
            if (this.dteSagyoYMDFrom.Value != null)
            {
                arginfo.SagyoYMDTo = this.dteSagyoYMDTo.Value.Value;
            }
            else
            {
                arginfo.SagyoYMDTo = DateTime.MinValue;
            }
        ...
        private void StartReportCreate()
        {
            lock (this)
            {
                ...
                //印刷用データを取得
                this._RptList = this._HibetsuSagyoKeikakuhyouRpt.GetList(this._ArgInfo);
