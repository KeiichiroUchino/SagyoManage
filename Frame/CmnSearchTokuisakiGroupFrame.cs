...
{
    public partial class CmnSearchTokuisakiGroupFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "得意先グループ検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private TokuisakiGroup searchTblClass;

        /// <summary>
        /// 得意先グループ情報のリストを保持する領域
        /// </summary>
        private IList<TokuisakiGroupInfo> TokuisakiGroupList = null;
        ...
        /// 使用停止列番号
        /// </summary>
        private const int COL_MUKO = 2;
        /// <summary>
        /// 検索一覧最大列数
        /// </summary>
        private const int COL_MAXCOLNUM_LIST = 3;
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public CmnSearchTokuisakiGroupFrame()
        {
            InitializeComponent();
        }
        ...
        private void InitCmnSearchTokuisakiGroupFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new TokuisakiGroup();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtTokuisakiGroupMei, ctl => ctl.Text, this.edtTokuisakiGroupMei_Validating));
        }
        ...
        {
            //得意先グループ一覧を取得
            this.TokuisakiGroupList =
                this.searchTblClass.GetList(new TokuisakiGroupSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.TokuisakiGroupList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<TokuisakiGroupInfo> wk_list =
                    this.GetMatchedList(this.TokuisakiGroupList).OrderBy(x => x.TokuisakiGroupCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.TokuisakiGroupCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.TokuisakiGroupName);
        ...
        private List<TokuisakiGroupInfo> GetMatchedList(IList<TokuisakiGroupInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<TokuisakiGroupInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (TokuisakiGroupInfo item in wk_list)
            {
                //「コード」+「名称」であいまい検索
                string item_mei =
        ...
        /// <summary>
        /// 選択している得意先グループ情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<TokuisakiGroupInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// <summary>
        /// 一覧にて選択中の得意先グループ情報を取得します。
        /// </summary>
        /// <returns>得意先グループ情報</returns>
        private List<TokuisakiGroupInfo> GetInfoByListOnSelection()
        {
            List<TokuisakiGroupInfo> rt_list = new List<TokuisakiGroupInfo>();
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    for (int i = 0; i < item.RowCount; i++)
                    {
                        TokuisakiGroupInfo info =
                            ((TokuisakiGroupInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public TokuisakiGroupInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された得意先グループ情報を一覧で取得します。
        /// </summary>
        public List<TokuisakiGroupInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchTokuisakiGroupFrame();
        }
        ...
        private void CmnSearchTokuisakiGroupFrame_KeyDown(object sender, KeyEventArgs e)
        {
