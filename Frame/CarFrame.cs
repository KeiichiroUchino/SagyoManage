...
{
    public partial class CarFrame : Form, IFrameBase
    {
        #region コンストラクタ
        ...
        /// 画面のタイトル
        /// </summary>
        private const string WINDOW_TITLE = "車両登録";
        ...
        /// 車両コード列番号
        /// </summary>
        private const int COL_CAR_CODE = 0;

        /// <summary>
        /// 車番列番号
        /// </summary>
        private const int COL_CAR_LICPLATECARNO = 1;

        /// <summary>
        /// 車両名列番号
        /// </summary>
        private const int COL_CAR_NAME = 2;

        /// <summary>
        /// 車両リスト最大列数
        /// </summary>
        private const int COL_MAXCOLNUM_CARLIST = 3;
        ...
        private void InitCarFrame()
        {
            ...
            //車両クラスインスタンス作成
            this._Car = new Car(this.appAuth);
        ...
        /// メニュー関連の初期化をします。
        /// </summary>
        private void InitMenuCar()
        {
            // 操作メニュー
            this.InitActionMenuCars();
        }

        /// <summary>
        /// 操作メニューを初期化します。
        /// </summary>
        private void InitActionMenuCars()
        {
            //操作メニューのクラスをインスタンス化
        ...
        /// 車両リストを初期化します。
        /// </summary>
        private void InitCarListSheet()
        {
            //SheetView変数定義
            SheetView sheet0 = this.fpCarListGrid.Sheets[0];

            //行数の初期化
            DefaultSheetDataModel dataModel =
                new DefaultSheetDataModel(0, COL_MAXCOLNUM_CARLIST);
        ...
        {
            this.numCarCode.Value = 0;
            this.edtLicPlate.Text = string.Empty;
        ...
        {
            //車両スタイル情報初期化
            this.InitCarStyleInfo();
        }

        /// <summary>
        /// 車両のスタイル情報を格納するメンバ変数の初期化をします。
        /// </summary>
        private void InitCarStyleInfo()
        {
            //SheetView変数定義
            SheetView sheet0 = this.fpCarListGrid.Sheets[0];

            //デザイナから各列のStyleInfoを取得してメンバに保持
            this.initialCarStyleInfoArr = new StyleInfo[COL_MAXCOLNUM_CARLIST];

            for (int i = 0; i < COL_MAXCOLNUM_CARLIST; i++)
            {
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.numYoshasakiCode, ctl => ctl.Text, this.numYoshasakiCode_Validating));
        }
        ...
        ///// 傭車先コードの値を取得します。
        ///// </summary>
        //private int YoshasakiCode
        //{
        //    //get { return Convert.ToInt32(this.numYoshasakiCode.Value); }
        //}
        ...
        private void ChangeMode(FrameEditMode mode)
        {
            switch (mode)
            {
                case FrameEditMode.Default:
                    ...
                    this.numCarCode.Enabled = false;
                    this.edtLicPlate.Enabled = false;
                    //this.edtLicPlateCarNo.Enabled = false;
                ...
                case FrameEditMode.New:
                    ...
                    this.pnlBottom.Enabled = true;
                    this.numCarCode.Enabled = true;
                    this.edtLicPlate.Enabled = true;
                    //--ファンクションの使用可否
                    _commandSet.ChangeCode.Enabled = false;
        ...
        private void SetCarListSheet()
        {
            ...
            try
            {
                ...
                IList<CarInfo> list = this.GetMatchedList(this._CarInfoList);

                //使用停止表示フラグによってデータ再抽出
                IList<CarInfo> wk_list =
                ...
                //Spreadのデータモデルを作る
                DefaultSheetDataModel datamodel =
                    new DefaultSheetDataModel(wk_list.Count, COL_MAXCOLNUM_CARLIST);

                //Spreadのスタイルモデルの作成
                DefaultSheetStyleModel stylemodel =
                    new DefaultSheetStyleModel(wk_list.Count, COL_MAXCOLNUM_CARLIST);

                //ループしてデータモデルにセット
                for (int j = 0; j < wk_list.Count; j++)
                {
                    //1件分を取得しておく
                    CarInfo wk_info = wk_list[j];

                    //スタイルモデルのセット
                    for (int k = 0; k < COL_MAXCOLNUM_CARLIST; k++)
                    {
                    ...
                    datamodel.SetValue(j, COL_CAR_CODE, wk_info.CarCode);
                    datamodel.SetValue(j, COL_CAR_LICPLATECARNO, wk_info.LicPlateCarNo);
                    datamodel.SetValue(j, COL_CAR_NAME, wk_info.CarName);

                    datamodel.SetTag(j, COL_CAR_CODE, wk_list[j]);
                }

                //Spreadにデータモデルをセット
                this.fpCarListGrid.Sheets[0].Models.Data = datamodel;

                //Spreadにスタイルモデルをセット
                this.fpCarListGrid.Sheets[0].Models.Style = stylemodel;

                //選択行にフォーカスを合わせて選択状態にする
                this.fpCarListGrid.Sheets[0].SetActiveCell(selectrowidx_Result, 0, true);

                //選択行にスクロールバーを移動させる
                this.fpCarListGrid.ShowActiveCell(
                ...
                if (dataCount > 0)
                {
                    this.fpCarListGrid.Sheets[0].AddSelection(selectrowidx_Result, -1, 1,
                        this.fpCarListGrid.Sheets[0].ColumnCount - 1);
        ...
        private List<CarInfo> GetMatchedList(IList<CarInfo> list)
        {
            ...
            foreach (CarInfo item in list)
            {
                //「コード」+「車番」であいまい検索
                string item_mei =
                    Convert.ToInt32(item.CarCode) + Environment.NewLine
                    + item.LicPlateCarNo.Trim() + Environment.NewLine
                    + item.CarName.Trim() + Environment.NewLine;
        ...
        {
            //選択車両情報をリストから取得して設定
            CarInfo info = this.GetCarInfoByCarListOnSelection();

            if (info == null || info.CarId == 0)
            {
                this.DoStartNewData();
            }
            else 
            {
                try
                {
                    //車両情報取得
                    this._CarInfo =
                        this._Car.GetInfoById(info.CarId);
        ...
        /// <summary>
        /// 車両リストにて選択中の車両情報を取得します。
        /// 未選択の場合は初期値を返却します。
        /// </summary>
        /// <returns>選択中の車両情報</returns>
        private CarInfo GetCarInfoByCarListOnSelection()
        {
            //返却用
            CarInfo rt_info = null;
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                if (rowcount > 0)
                {
                    //最初の列にセットしたTagから車両情報を取得
                    rt_info = ((CarInfo)sheet0.Cells[select_row, COL_CAR_CODE].Tag);
                }
            }
        ...
        private void SetScreen()
        {
            if (this._CarInfo == null)
            {
                ...
                this._CarInfo = new CarInfo();

                //選択された車両情報を取得
                CarInfo info = this.GetCarInfoByCarListOnSelection();
                this._CarInfo.CarId = info.CarId;

                //表示項目設定
                this.numCarCode.Value = info.CarCode;
                this.edtLicPlate.Text = Convert.ToString(info.LicPlateCarNo);
                this.edtCarName.Text = Convert.ToString(info.CarName);
            }
            else
            {
                this.numCarCode.Value = Convert.ToInt32(_CarInfo.CarCode);
                this.edtLicPlate.Text = Convert.ToString(_CarInfo.LicPlateCarNo);
                this.edtCarName.Text = Convert.ToString(_CarInfo.CarName);
                this.chkDisableFlag.Checked = _CarInfo.DisableFlag;
            }
        }
        ...
        private void DoUpdate()
        {
            ...
            if (this.ValidateChildren(ValidationConstraints.None) && this.CheckInputs())
            {
                ...
                try
                {
                    ...
                    {
                        this._Car.Save(tx, this._CarInfo);
                    });

                    //操作ログ(保存)の条件取得
                    string log_jyoken =
                        FrameUtilites.GetDefineLogMessage(
                        "C10002", new string[] { "車両", _CarInfo.CarCode.ToString() });
                ...
                catch (Model.DALExceptions.UniqueConstraintException ex)
                {
                    ...
                    {
                        this.numCarCode.Focus();
                    }
                    else
                    {
                        this.edtLicPlate.Focus();
                    }
                }
            }
        }
        ...
        {
            _CarInfo.CarCode = Convert.ToInt32(this.numCarCode.Value);
            _CarInfo.LicPlateCarNo = this.edtLicPlate.Text;
            _CarInfo.CarName = this.edtCarName.Text;
            _CarInfo.DisableFlag = this.chkDisableFlag.Checked;
        }
        ...
        private bool CheckInputs()
        {
            ...
            //コードの必須入力チェック
            if (rt_val && Convert.ToDecimal(this.numCarCode.Value) == 0)
            {
                rt_val = false;
        ...
        {
            //if (this.ActiveControl == this.numYoshasakiCode)
            //{
            //    this.ShowCmnSearchYoshasaki();
            //}
        }
        ...
        /// </summary>
        /// <param name="e"></param>
        private void ValidateYoshasakiCode(CancelEventArgs e)
        {
            bool is_clear = false;
            ...
            try
            {
                ...
                //    this._DalUtil.Torihikisaki.GetInfo(this.YoshasakiCode);
                ...
                //    else
                //    {
                //        //this.numYoshasakiCode.Tag = info.ToraDONTorihikiId;
                //        //this.numYoshasakiCode.Value = info.ToraDONTorihikiCode;
                //        //this.edtYoshasakiName.Text = info.ToraDONTorihikiName;
                //    }
                //}
            }
            finally
            {
                if (is_clear)
                {
                    //this.numYoshasakiCode.Tag = null;
                    //this.numYoshasakiCode.Value = null;
                    //this.edtYoshasakiName.Text = string.Empty;
                }
            }
        }
        ...
        /// </summary>
        /// <param name="e"></param>
        private void ProcessFpCarListGridPreviewKeyDown(PreviewKeyDownEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Enter:
        ...
        private void CarFrame_FormClosing(object sender, FormClosingEventArgs e)
        {
            //フォームクローズ時
            this.ConfirmClose(e);
        ...
        private void CarFrame_Shown(object sender, EventArgs e)
        {
            //ログ出力
            FrameLogWriter.GetLogger(this.appAuth).LoggingLog(
        ...
        private void fpCarListGrid_CellDoubleClick(object sender, CellClickEventArgs e)
        {
            //車両リスト - CellDoubleClick
        ...
        /// <param name="e"></param>
        private void fpCarListGrid_CellClick(object sender, CellClickEventArgs e)
        {
            if (e.Button == MouseButtons.Left && e.ColumnHeader)
            {
                this.fpCarListGrid.Sheets[0].AutoSortColumn(e.Column);
            }
        }

        private void fpCarListGrid_PreviewKeyDown(object sender, PreviewKeyDownEventArgs e)
        {
            //登録済リスト - PreviewKeyDown
            this.ProcessFpCarListGridPreviewKeyDown(e);
        }

        private void CarFrame_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void numYoshasakiCode_Validating(object sender, CancelEventArgs e)
        {
            //傭車先コード
            this.ValidateYoshasakiCode(e);
        }

        private void fpCarListGrid_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            this.DoGetData(false);
        }
