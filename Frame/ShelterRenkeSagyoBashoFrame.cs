...
{
    public partial class ShelterRenkeSagyoBashoFrame : Form, IFrameBase
    {
        #region コンストラクタ
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public ShelterRenkeSagyoBashoFrame()
        {
            InitializeComponent();
        }
        ...
        /// 画面のタイトル
        /// </summary>
        private const string WINDOW_TITLE = "外部システム連携（Shelter）作業場所";
        ...
        /// 作業場所パラメータリスト
        /// </summary>
        private List<SagyoBashoInfo> _SagyoBashoInfoList;
        ...
        {
            //メンバをクリア
            this._SagyoBashoInfoList = new List<SagyoBashoInfo>();
        ...
        private bool LoadCsvData(List<List<string>> csvlines, SqlTransaction transaction)
        {
            ...
            //得意先コードリスト
            List<string> codeListTokuisaki = new List<string>();
            ...
            for (int i = 0; i < csvlines.Count(); i++)
            {
                ...
                try
                {
                    ...
                    {
                        codeList.Add(FrameUtilites.SubstringShiftJIS(line[(int)CsvIndex.SagyoBashoCode], 1, 8));
                    }

                    if (!string.IsNullOrWhiteSpace(line[(int)CsvIndex.TokuisakiCode]))
                    {
                        codeListTokuisaki.Add(FrameUtilites.SubstringShiftJIS(line[(int)CsvIndex.TokuisakiCode], 1, 7));
                    }
                }
            ...
            // 作業場所一覧取得（行ロック）
            List <SagyoBashoInfo> list = this._SagyoBasho.BulkGetListInternal(idList.Distinct().ToList(), transaction, true);

            // 得意先一覧取得
            List<TokuisakiInfo> tokuisakiList = this._DalUtil.Tokuisaki.BulkGetListInternal(codeListTokuisaki.Distinct().ToList());
            ...
            for (int i = 0; i < csvlines.Count(); i++)
            {
                ...
                try
                {
                    ...
                    //var addList = this._SagyoBashoInfoList.Where(x => x.SagyoBashoCode == sagyouBashoCd);
                    ...
                    if (!string.IsNullOrWhiteSpace(info.SagyoBashoCode))
                    {
                        //作業場所コードCSVファイル重複チェック
                        if (codeList.Where(x => x == info.SagyoBashoCode).Count() > 1)
                        {
                            errList.Add(string.Format("CSVファイルの{0}列目、{1}行目の値は、CSVファイル内に重複する作業場所コードが存在します。", (int)CsvIndex.SagyoBashoCode + 1, i + 1));
                        }
                        ...
                        {
                            List<SagyoBashoInfo> tlist =
                                this._SagyoBasho.GetListInternal(transaction,
                                new SagyoBashoSearchParameter { SagyoBashoCode = info.SagyoBashoCode, ShelterId = chkId, DuplicateFlg = true }).ToList();

                            if (tlist.Count() > 0)
                            {
                                errList.Add(string.Format("CSVファイルの{0}列目、{1}行目の値は、別の作業場所情報で、すでに作業場所コードが使用済みです。",
                                    (int)CsvIndex.SagyoBashoCode + 1, i + 1));
                            }
                        }
                    }
                    ...
                    {
                        //得意先コードが登録されている場合
                        var tokuisakiInfo = tokuisakiList.Where(x => x.TokuisakiCode == FrameUtilites.SubstringShiftJIS(line[(int)CsvIndex.TokuisakiCode], 1, 7));
                        if (tokuisakiInfo.Count() > 0)
                        {
                            info.TokuisakiId = tokuisakiInfo.First().TokuisakiId;
                        }
                        //得意先コードが登録されていない場合
                        else
                        {
                            ...
                            errList.Add(string.Format("CSVファイルの{0}列目、{1}行目の値は、得意先として登録されていません。", (int)CsvIndex.TokuisakiCode + 1, i + 1));

                        }
                    }

                    //作業場所コード必須チェック
                    FrameUtilites.isNullCheckCsvColumn(line, i, (int)CsvIndex.SagyoBashoName, errList);
                    info.SagyoBashoName = FrameUtilites.SubstringShiftJIS(line[(int)CsvIndex.SagyoBashoName], 1, 70);

                    info.SagyoBashoAddress1 = string.Empty;
                    info.SagyoBashoAddress2 = string.Empty;

                    this._SagyoBashoInfoList.Add(info);
                }
                catch (CanRetryException err)
                {
            ...
            if (errList.Count() > 0)
            {
                try
                {
                    ...
                    if (Directory.Exists(path))
                    {
                        try
                        {
                            string fileName = "作業場所情報エラーファイル_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".txt";
                ...
                if (this.ValidateChildren(ValidationConstraints.None))
                {
                    try
                    {
                        ...
                        SQLHelper.ActionWithTransaction(tx =>
                        {
                            ...
                            // 登録・更新
                            this._SagyoBasho.BulkSave(tx, this._SagyoBashoInfoList);
                        });
