...
{
    public partial class CmnSearchItemFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "品目検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Item searchTblClass;

        /// <summary>
        /// 品目情報のリストを保持する領域
        /// </summary>
        private IList<ItemInfo> ItemList = null;
        ...
        private void InitCmnSearchItemFrame()
        {
            //画面タイトルの設定
        ...
        private void DoGetData()
        {
            //品目一覧を取得
            this.ItemList =
                this.searchTblClass.GetList(new ItemSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                //抽出後のリストを取得（同時に並べ替え）
                List<ItemInfo> wk_list =
                    this.GetMatchedList(this.ItemList).OrderBy(x => x.ToraDONItemCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.ToraDONItemCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.ToraDONItemName);
                    datamodel.SetValue(j, COL_KANA, wk_info.ToraDONItemNameKana);
        ...
        /// <summary>
        /// 選択している品目情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<ItemInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        private List<ItemInfo> GetInfoByListOnSelection()
        {
            List<ItemInfo> rt_list = new List<ItemInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        ItemInfo info =
                            ((ItemInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public ItemInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された品目情報を一覧で取得します。
        /// </summary>
        public List<ItemInfo> SelectedInfoList { get; private set; }
        ...
        private void CmnSearchItemFrame_KeyDown(object sender, KeyEventArgs e)
        {
