...
{
    public partial class CmnSearchFigFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "単位検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private ToraDONFig searchTblClass;

        /// <summary>
        /// 単位情報のリストを保持する領域
        /// </summary>
        private IList<ToraDONFigInfo> FigList = null;
        ...
        private void InitCmnSearchFigFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new ToraDONFig();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtFigMei, ctl => ctl.Text, this.edtFigMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //単位一覧を取得
            this.FigList =
                this.searchTblClass.GetList(new FigSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.FigList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<ToraDONFigInfo> wk_list =
                    this.GetMatchedList(this.FigList).OrderBy(x => x.FigCode).ToList();
                ...
                {
                    ToraDONFigInfo wk_info = wk_list[j];
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.FigCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.FigName);
        ...
        private List<ToraDONFigInfo> GetMatchedList(IList<ToraDONFigInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<ToraDONFigInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (ToraDONFigInfo item in wk_list)
            {
                //「コード」+「名称」であいまい検索
                string item_mei =
                    Convert.ToInt32(item.FigCode) + Environment.NewLine
                    + item.FigName.Trim() + Environment.NewLine;
        ...
        /// <summary>
        /// 選択している単位情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<ToraDONFigInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// 一覧にて選択中の単位情報を取得します。
        /// </summary>
        /// <returns>単位情報</returns>
        private List<ToraDONFigInfo> GetInfoByListOnSelection()
        {
            List<ToraDONFigInfo> rt_list = new List<ToraDONFigInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        ToraDONFigInfo info =
                            ((ToraDONFigInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public ToraDONFigInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された単位情報を一覧で取得します。
        /// </summary>
        public List<ToraDONFigInfo> SelectedInfoList { get; private set; }
        ...
        private void CmnSearchFigFrame_KeyDown(object sender, KeyEventArgs e)
        {
