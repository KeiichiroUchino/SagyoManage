...
{
    public partial class CmnSearchHanroGroupFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "販路グループ検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private HanroGroup searchTblClass;

        /// <summary>
        /// 販路グループ情報のリストを保持する領域
        /// </summary>
        private IList<HanroGroupInfo> HanroGroupList = null;
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public CmnSearchHanroGroupFrame()
        {
            InitializeComponent();
        }
        ...
        private void InitCmnSearchHanroGroupFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new HanroGroup();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtHanroGroupMei, ctl => ctl.Text, this.edtHanroGroupMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //販路グループ一覧を取得
            this.HanroGroupList =
                this.searchTblClass.GetList(new HanroGroupSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.HanroGroupList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<HanroGroupInfo> wk_list =
                    this.GetMatchedList(this.HanroGroupList).OrderBy(x => x.HanroGroupCode).ToList();
                ...
                for (int j = 0; j < wk_list.Count; j++)
                {
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.HanroGroupCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.HanroGroupName);
        ...
        private List<HanroGroupInfo> GetMatchedList(IList<HanroGroupInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<HanroGroupInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (HanroGroupInfo item in wk_list)
            {
                //「コード」+「名称」であいまい検索
                string item_mei =
        ...
        /// <summary>
        /// 選択している販路グループ情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<HanroGroupInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// <summary>
        /// 一覧にて選択中の販路グループ情報を取得します。
        /// </summary>
        /// <returns>販路グループ情報</returns>
        private List<HanroGroupInfo> GetInfoByListOnSelection()
        {
            List<HanroGroupInfo> rt_list = new List<HanroGroupInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        HanroGroupInfo info =
                            ((HanroGroupInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public HanroGroupInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された販路グループ情報を一覧で取得します。
        /// </summary>
        public List<HanroGroupInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchHanroGroupFrame();
        }
        ...
        private void CmnSearchHanroGroupFrame_KeyDown(object sender, KeyEventArgs e)
        {
