...
{
    public partial class CmnSearchOwnerFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "荷主検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private ToraDONOwner searchTblClass;

        /// <summary>
        /// 荷主情報のリストを保持する領域
        /// </summary>
        private IList<ToraDONOwnerInfo> OwnerList = null;
        ...
        private void InitCmnSearchOwnerFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new ToraDONOwner();
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtOwnerMei, ctl => ctl.Text, this.edtOwnerMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //荷主一覧を取得
            this.OwnerList =
                this.searchTblClass.GetList(new ToraDONOwnerSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                //抽出後のリストを取得（同時に並べ替え）
                List<ToraDONOwnerInfo> wk_list =
                    this.GetMatchedList(this.OwnerList).OrderBy(x => x.OwnerCode).ToList();
                ...
                {
                    ToraDONOwnerInfo wk_info = wk_list[j];
                    ...
                    //データモデルのセット
                    datamodel.SetValue(j, COL_CODE, wk_info.OwnerCode);
                    datamodel.SetValue(j, COL_NAME, wk_info.OwnerName);
                    datamodel.SetValue(j, COL_SNAME, wk_info.OwnerShortName);
                    datamodel.SetValue(j, COL_KANA, wk_info.OwnerNameKana);
        ...
        private List<ToraDONOwnerInfo> GetMatchedList(IList<ToraDONOwnerInfo> list)
        {
            ...
            //使用停止表示フラグによってデータ再抽出
            IList<ToraDONOwnerInfo> wk_list =
                this.chkAllFlag.Checked ? list : list.Where(x => !x.DisableFlag).ToList();

            foreach (ToraDONOwnerInfo item in wk_list)
            {
                //「コード」+「名称」+「略称」+「カナ」であいまい検索
                string item_mei =
                    Convert.ToInt32(item.OwnerCode) + Environment.NewLine
                    + item.OwnerName.Trim() + Environment.NewLine
                    + item.OwnerShortName.Trim() + Environment.NewLine
                    + item.OwnerNameKana.Trim() + Environment.NewLine;
        ...
        /// <summary>
        /// 選択している荷主情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<ToraDONOwnerInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        /// 一覧にて選択中の荷主情報を取得します。
        /// </summary>
        /// <returns>荷主情報</returns>
        private List<ToraDONOwnerInfo> GetInfoByListOnSelection()
        {
            List<ToraDONOwnerInfo> rt_list = new List<ToraDONOwnerInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        ToraDONOwnerInfo info =
                            ((ToraDONOwnerInfo)sheet0.Cells[item.Row + i, COL_CODE].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public ToraDONOwnerInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された荷主情報を一覧で取得します。
        /// </summary>
        public List<ToraDONOwnerInfo> SelectedInfoList { get; private set; }
        ...
        private void CmnSearchOwnerFrame_KeyDown(object sender, KeyEventArgs e)
        {
