...
{
    public partial class CmnSearchTokuisakiFrame : Form, IFrameBase
    {
        #region ユーザ定義
        ...
        /// 画面タイトルの静的な部分
        /// </summary>
        private const string WINDOW_TITLE = "得意先検索";
        ...
        /// 検索テーブルのクラス領域
        /// </summary>
        private Jpsys.SagyoManage.SQLServerDAL.Tokuisaki searchTblClass;

        /// <summary>
        /// 得意先情報のリストを保持する領域
        /// </summary>
        private IList<TokuisakiInfo> TokuisakiList = null;
        ...
        /// 本クラスのデフォルトコンストラクタです。
        /// </summary>
        public CmnSearchTokuisakiFrame()
        {
            InitializeComponent();
        }
        ...
        private void InitCmnSearchTokuisakiFrame()
        {
            //画面タイトルの設定
            ...
            //検索テーブルのクラスインスタンス作成
            this.searchTblClass = new Jpsys.SagyoManage.SQLServerDAL.Tokuisaki();
        ...
        /// メニュー関連を初期化します。
        /// </summary>
        private void InitMenuTokuisaki()
        {
            //操作メニュー
        ...
        private void SetupValidatingEventRaiser()
        {
            ...
            //    ControlValidatingEventRaiser.Create(this.edtTokuisakiMei, ctl => ctl.Text, this.edtTokuisakiMei_Validating));
        }
        ...
        private void DoGetData()
        {
            //得意先一覧を取得
            this.TokuisakiList =
                this.searchTblClass.GetList(new TokuisakiSearchParameter() { });
        }
        ...
        private void SetListSheet()
        {
            ...
            try
            {
                ...
                if (this.TokuisakiList == null)
                {
                    return;
                }

                //抽出後のリストを取得（同時に並べ替え）
                List<TokuisakiInfo> wk_list =
                    this.GetMatchedList(this.TokuisakiList).OrderBy(x => x.TokuisakiCode).ToList();
                ...
                {
                    TokuisakiInfo wk_info = wk_list[j];
                    ...
                    datamodel.SetValue(j, (int)SpreadColKeys.TokuisakiAddress1, wk_info.TokuisakiAddress1);
                    datamodel.SetValue(j, (int)SpreadColKeys.TokuisakiAddress2, wk_info.TokuisakiAddress2);
                    datamodel.SetValue(j, (int)SpreadColKeys.ShiyoTeishi, wk_info.ShiyoTeishi);

                    // Tagへ1件分のデータを退避（後で使用）
                    datamodel.SetTag(j, (int)SpreadColKeys.TokuisakiCode, wk_info);
                }
        ...
        private List<TokuisakiInfo> GetMatchedList(IList<TokuisakiInfo> list)
        {
            List<TokuisakiInfo> rt_list = new List<TokuisakiInfo>();
            ...
            foreach (TokuisakiInfo item in wk_list)
            {
                //「コード」+「名称」+「カナ」であいまい検索
                string item_mei =
                    item.TokuisakiCode.Trim() + Environment.NewLine
                    + item.TokuisakiName.Trim() + Environment.NewLine
                    + item.TokuisakiAddress1.Trim() + Environment.NewLine
                    + item.TokuisakiAddress2.Trim() + Environment.NewLine;
        ...
        /// <summary>
        /// 選択している得意先情報をメンバにセットします。
        /// </summary>
        private void SetSelectedInfo()
        {
            List<TokuisakiInfo> wk_val = this.GetInfoByListOnSelection();
        ...
        private List<TokuisakiInfo> GetInfoByListOnSelection()
        {
            List<TokuisakiInfo> rt_list = new List<TokuisakiInfo>();
            ...
            if (sheet0.SelectionCount > 0)
            {
                ...
                foreach (CellRange item in sheet0.GetSelections())
                {
                    ...
                    {
                        TokuisakiInfo info =
                            ((TokuisakiInfo)sheet0.Cells[item.Row + i, (int)SpreadColKeys.TokuisakiCode].Tag);
        ...
        /// 選択された検索テーブル情報を取得します。
        /// </summary>
        public TokuisakiInfo SelectedInfo { get; private set; }

        /// <summary>
        /// 選択された得意先情報を一覧で取得します。
        /// </summary>
        public List<TokuisakiInfo> SelectedInfoList { get; private set; }
        ...
        {
            this.InitCmnSearchTokuisakiFrame();
        }
        ...
        private void CmnSearchTokuisakiFrame_KeyDown(object sender, KeyEventArgs e)
        {
